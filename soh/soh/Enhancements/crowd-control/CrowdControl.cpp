...
extern "C" void OnMessageCallback(const char* data) {
    SPDLOG_TRACE("[TCPListener] CC callback");
    CrowdControl::Instance->OnMessage(data);
}
...
extern "C" void CrowdControl::OnMessage(const char* data) {
    Effect* incomingEffect = ParseMessage(data);
    if (!incomingEffect) {
        return;
    }
